...
void main(List<String> args) {
  ...
  for (int i = 1; i <= solutions.length; i++) {
    ...
    try {
      ...
      final List<double> inputs = stdin.map(double.parse).toList();
      final List<double> outputs = stdout.map(double.parse).toList();

      for (final double input in inputs) {
        final double expected = outputs[inputs.indexOf(input)];
        final double output = solution(input);
        if (output != expected) {
          throw Exception(
