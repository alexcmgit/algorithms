...
class SolutionTest(unittest.TestCase):
    ...
    def assert_solution_expected_out(self, solution, id):
        for i in range(0, len(self.inputs), 3):
            annual_salary, percent_to_save, dream_home_cost = (self.inputs[i], self.inputs[i + 1], self.inputs[i + 2])
            expected = self.outputs[i // 3]
            ...
            required_months = exec_solution(solution, float(annual_salary), float(percent_to_save.replace('%', '')) / 100, float(dream_home_cost))

            self.assertEqual(required_months, float(expected), f'Failed for solution {id}: current value of [required_months]: {required_months}, expected = {expected}')

            print(F'Solution {id}, Case {i // 3 + 1} OK.')
