...
class Message(object):
    ...
    def build_shift_dict(self, shift: int):
        ...
        for i, e in enumerate(string.ascii_lowercase):
            mapper[e] = string.ascii_lowercase[(i + shift) % len(string.ascii_lowercase)]

        for i, e in enumerate(string.ascii_uppercase):
            mapper[e] = string.ascii_uppercase[(i + shift) % len(string.ascii_uppercase)]
    ...
    def apply_shift(self, shift: int) -> str:
        ...
        return ''.join([mapper.get(c, c) for c in self.message_text])
...
class CiphertextMessage(Message):
    ...
    def _get_alpha_from_message(self, shift: int) -> str:
        '''
        If [shift] = "Hello, Word"
        Returns "hello world"
        '''
        return ''.join([c if c == ' ' or c.isalpha() else '' for c in self.apply_shift(shift)]).lower()
    ...
    def decrypt_message(self):
        ...
        for i in range(0, 26):
            output = self.apply_shift(i)
            valid_words_count = self._valid_words_count(self._get_alpha_from_message(i))

            if valid_words_count > words:
                words = valid_words_count
                message = output
                shift = i
