...
class SolutionTest(unittest.TestCase):
    def setUp(self) -> None:
        self.solutions = map(lambda e: (e[1], __import__(e[0]).multiples_of_3_and_5_below), solutions())
        self.inputs = list(map(int, stdin()))
        self.outputs = list(map(int, stdout()))

    def test_solutions(self):
        for id, solution in self.solutions:
            self.assert_solution_expected_output(solution, f'{id}')
            
    def assert_solution_expected_output(self, solution, id: str):
